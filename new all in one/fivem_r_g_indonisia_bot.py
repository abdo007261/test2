...
TOKEN = getenv("7421453509:AAFWJNw2xRejJw7UpxLF1fOGVaIyjtyTa4c")
TOKEN = "7421453509:AAFWJNw2xRejJw7UpxLF1fOGVaIyjtyTa4c"
...
TARGET_CHANNEL_ID = "-1002183889362"
exit_flag = False
        ...
        async def main2():
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        async with Bot(
                            ...
                            channels_list = [-1001817370976, -1002183889362]
            ...
            def get_redis_connection():
                try:
                    ...
                    # print(f"[ID] Redis connection failed: {e}. Using direct API fallback.")
                    return None
                ...
                try:
                    ...
                    if r:
                        ...
                        if shared_data:
                            ...
                            # print(f"[ID] Shared data read from Redis successfully (updated {int(time.time() * 1000) - shared_info['timestamp']}ms ago)")
                            ...
                            if int(time.time() * 1000) - shared_info['timestamp'] < 10000:
                                ...
                                # print("[ID] Redis data is stale, checking local file")
                                return read_local_file()
                        else:
                            # Fallback to old key for backward compatibility
                            raw_data = r.get("api_response")
                            if raw_data:
                                # print("[ID] Data read from Redis (legacy key) successfully")
                                return json.loads(raw_data)
                            else:
                                # print("[ID] No data in Redis, checking local file")
                                return read_local_file()
                    else:
                        # print("[ID] Redis unavailable, checking local file")
                        return read_local_file()
                except Exception as e:
                    # print(f"[ID] Failed to read from Redis: {e}. Checking local file.")
                    return read_local_file()
                ...
                try:
                    if os.path.exists("fivem_shared_data.json"):
                        ...
                        if current_time - shared_data["timestamp"] < 30000:
                            # print(f"[ID] Data read from local file successfully (updated {current_time - shared_data['timestamp']}ms ago)")
                            return shared_data["game_data"]
                        else:
                            # print("[ID] Local file data is stale, using direct API")
                            return None
                    else:
                        print("[ID] No local file found, using direct API")
                        return None
                        
                except Exception as e:
                    print(f"[ID] Failed to read local file: {e}")
                    return None
            ...
            def fetch_data():
                ...
                # print("[ID] Using direct API call as Redis fallback")
                ...
                for attempt in range(MAX_RETRIES):
                    try:
                        ...
                        print(f"[ID] ⚠ Connection issue detected: {e}. Retrying...")
                        if attempt < MAX_RETRIES - 1:
                            time.sleep(2)
                        else:
                            print(f"[ID] ❌ Max retries reached for API call")
                            return None

                    except requests.exceptions.RequestException as e:
                        print(f"[ID] ❌ API request error: {e}")
                        if attempt < MAX_RETRIES - 1:
                            time.sleep(2)
                        else:
                            return None
                ...
                if data and 'data' in data and 'records' in data['data']:
                    ...
                    if records:
                        ...
                        if latest_issue is None or last_record[
                            ...
                            colors = ("🟢 Hijau" if int(value_number) %
                                      2 else "🔴 Merah")
            ...
            async def main_async():
                ...
                while not exit_flag:
                    ...
                    if consecutive_losses > 1 or consecutive_losses == 1:
                        ...
                        message1 = f'''
✨Merah-Hijau  5 menit✨
Periode:  {extracted_time}
Beli : {trade_selection}
Tingkat kemenangan: 80%
Banyak:  x{current_bet_value}
Menggunakan paket 3X, dapatkan bunga 30% untuk setiap setoran  👉🏻Semakin tinggi stage, semakin banyak keuntungan yang Anda dapatkan🎉🎉'''
                        ...
                        if trade_selection == "🟢 Hijau":
                            trade_selection = "🔴 Merah"
                        else:
                            trade_selection = "🟢 Hijau"
                        current_bet_value = 1
                        result = f'RESUALT: {colors}'
...
Tingkat kemenangan: 80%
Banyak:  x{current_bet_value}
Menggunakan paket 3X, dapatkan bunga 30% untuk setiap setoran  👉🏻Semakin tinggi stage, semakin banyak keuntungan yang Anda'''
                        await send_message_to_telegram(result)
                        time.sleep(0.1)
                        await send_message_to_telegram(message)
                        time.sleep(0.1)
