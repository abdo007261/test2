...
TOKEN = getenv("7367367756:AAEo_fQyYQqKjmUxlXocMh_HTnEYXbjkbeg")
TOKEN = "7367367756:AAEo_fQyYQqKjmUxlXocMh_HTnEYXbjkbeg"
...
TARGET_CHANNEL_ID = "-1002148779642"
exit_flag = False
        ...
        async def main2():
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        async with Bot(
                            ...
                            channels_list = [-1001897689102, -1002148779642]
            ...
            def get_redis_connection():
                try:
                    ...
                    # print(f"[VI] Redis connection failed: {e}. Using direct API fallback.")
                    return None
                ...
                try:
                    ...
                    if r:
                        ...
                        if shared_data:
                            ...
                            # print(f"[VI] Shared data read from Redis successfully (updated {int(time.time() * 1000) - shared_info['timestamp']}ms ago)")
                            ...
                            if int(time.time() * 1000) - shared_info['timestamp'] < 10000:
                                ...
                                # print("[VI] Redis data is stale, checking local file")
                                return read_local_file()
                        else:
                            # Fallback to old key for backward compatibility
                            raw_data = r.get("api_response")
                            if raw_data:
                                # print("[VI] Data read from Redis (legacy key) successfully")
                                return json.loads(raw_data)
                            else:
                                # print("[VI] No data in Redis, checking local file")
                                return read_local_file()
                    else:
                        # print("[VI] Redis unavailable, checking local file")
                        return read_local_file()
                except Exception as e:
                    # print(f"[VI] Failed to read from Redis: {e}. Checking local file.")
                    return read_local_file()
                ...
                try:
                    if os.path.exists("fivem_shared_data.json"):
                        ...
                        if current_time - shared_data["timestamp"] < 30000:
                            # print(f"[VI] Data read from local file successfully (updated {current_time - shared_data['timestamp']}ms ago)")
                            return shared_data["game_data"]
                        else:
                            # print("[VI] Local file data is stale, using direct API")
                            return None
                    else:
                        print("[VI] No local file found, using direct API")
                        return None
                        
                except Exception as e:
                    print(f"[VI] Failed to read local file: {e}")
                    return None
            ...
            def fetch_data():
                ...
                # print("[VI] Using direct API call as Redis fallback")
                ...
                for attempt in range(MAX_RETRIES):
                    try:
                        ...
                        print(f"[VI] ⚠ Connection issue detected: {e}. Retrying...")
                        if attempt < MAX_RETRIES - 1:
                            time.sleep(2)
                        else:
                            print(f"[VI] ❌ Max retries reached for API call")
                            return None

                    except requests.exceptions.RequestException as e:
                        print(f"[VI] ❌ API request error: {e}")
                        if attempt < MAX_RETRIES - 1:
                            time.sleep(2)
                        else:
                            return None
                ...
                if data and 'data' in data and 'records' in data['data']:
                    ...
                    if records:
                        ...
                        if latest_issue is None or last_record[
                            ...
                            colors = ("🟢 Xanh" if int(value_number) %
                                      2 else "🔴 Đỏ")
...
Kì xổ:  {extracted_time}
Mua: {trade_selection}
Tỉ lệ thắng: 80%
Số lượng:  x{current_bet_value}
