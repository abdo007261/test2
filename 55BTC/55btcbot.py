...
current_blade_auth = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9..B6sOGoXcodPl9h7hzI6kCjQdt9Vf-mr35lYytgyf4sZs6mywamVBu2ysWRenb05BVluzahEEtIh73IfIhF1hMg"
...
def login_to_55btc():
    ...
    try:
        ...
        print(f"[55BTC] Attempting login with username: {LOGIN_CREDENTIALS['username']}")
        response = requests.post(url, data=data, headers=headers, timeout=30)
        
        print(f"[55BTC] Login response status: {response.status_code}")
        print(f"[55BTC] Login response headers: {dict(response.headers)}")
        print(f"[55BTC] Login response body: {response.text}")
        
        if response.status_code == 200:
            try:
                response_data = response.json()
                print(f"[55BTC] Parsed response data: {response_data}")
                ...
                if response_data.get('code') == 200 and 'data' in response_data and 'access_token' in response_data['data']:
                    ...
                    print(f"[55BTC] Login successful, new token obtained: {new_token[:50]}...")
                    return new_token
                else:
                    print(f"[55BTC] Login failed: Invalid response structure or code: {response_data.get('code')}")
                    return None
            except json.JSONDecodeError as e:
                print(f"[55BTC] Failed to parse JSON response: {e}")
                return None
        else:
            print(f"[55BTC] Login failed with status {response.status_code}")
            return None
    ...
    if data == 'turn_on':
        ...
        async def main():
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if response.status_code == 401:
                            print(f"[55BTC] 401 Unauthorized detected, attempting relogin...")
                            new_token = login_to_55btc()
                            if new_token:
                                ...
                                response = requests.get(url, **kwargs)
                                if response.status_code == 200:
                                    print(f"[55BTC] Request successful after relogin")
                                    return response
                        
                        # Check for other error status codes
                        if response.status_code >= 400:
                            print(f"[55BTC] HTTP error {response.status_code}: {response.text}")
                        ...
                        print(f"[55BTC] Network error: {e}. Attempt {attempt+1}/{max_retries}")
                        if attempt < max_retries - 1:
                            time.sleep(2)
                        else:
                            print("[55BTC] Max retries reached, skipping this request.")
                            return None
            ...
            def fetch_data():
                ...
                if response is None:
                    print("[55BTC] fetch_data: No response received")
                    return None
                
                try:
                    data = response.json()
                    if 'data' not in data or 'oneMinCurrentIssueDetail' not in data['data']:
                        print(f"[55BTC] fetch_data: Invalid data structure: {data}")
                        return None
                    ...
                    print(f"[55BTC] fetch_data: Error parsing response: {e}")
                    return None
            ...
            def signal_data(start_time, end_time):
                ...
                response = robust_requests_get(url, headers=headers, timeout=30)
                if response is None:
                    print("[55BTC] signal_data: Failed to fetch data after retries. Skipping.")
                    return
                
                try:
                    data = response.json()
                    if 'data' not in data:
                        print(f"[55BTC] signal_data: Invalid data structure: {data}")
                        return
                except Exception as e:
                    print(f"[55BTC] signal_data: Error parsing response: {e}")
                    return
                # print("Data:", data)
